...
def upload_google_drive_file(
    ...
    try:
        if update:
            _file = service.files().update(
                fileId=file_id,
                body=file_metadata,
                media_body=media,
                supportsAllDrives=True,
                fields=('id'),
                addParents=parent_folder_id).execute()
        else:
        ...
        if e.content == b'Failed to parse Content-Range header.':
            print(
                f'Failed to upload file {source_full_path} due to invalid size')
        ...
        for index, key_name in enumerate(matching_file_names):
            ...
            if destination_folder_name:
                parent_folder_id = find_folder_id(
                    service, destination_full_path, drive)
